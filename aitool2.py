...
def fetch_signals():
    ...
    return [
        (1, datetime.datetime.now() - datetime.timedelta(hours=1), 'BUY', 0.75, 50000),
        (2, datetime.datetime.now() - datetime.timedelta(minutes=30), 'SELL', 0.65, 51000)
    ]

# Trading function (placeholder implementation)
def place_order(signal):
    if signal == 'BUY':
        return "Simulated BUY order placed"
    elif signal == 'SELL':
        return "Simulated SELL order placed"
    else:
